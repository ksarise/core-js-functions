...
function getArgumentsCount(funcs) {
  const count = funcs.map((func) => func.length);
  return count;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argsStringed = args.map((arg) => JSON.stringify(arg)).join(',');
    logFunc(`${func.name}(${argsStringed}) starts`);
    const result = func.apply(this, args);
    logFunc(`${func.name}(${argsStringed}) ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args) {
  return (...partArgs) => fn(...args, ...partArgs);
}
